...
def main():
    ...
    # 导出为 ONNX
    dummy_image = [torch.ones(1, 3, 800, 800, dtype=torch.float32)]
    input_names = ["input_0"]
    output_names = ['pred_logits', 'pred_boxes']
    onnx_io = io.BytesIO()
    torch.onnx.export(model, dummy_image[0], onnx_io,
                  input_names = input_names, output_names = output_names, export_params=True,
                  opset_version = 11, do_constant_folding = False)
    torch.onnx.export(model, dummy_image[0], "detr.onnx",
                  input_names = input_names, output_names = output_names, export_params = True,
                  opset_version = 11, do_constant_folding = False)
    # torch.onnx.export(model, dummy_input, "detr.onnx", verbose=True, opset_version=13,
    #                   input_names=input_names,
    #                   output_names=output_names)
    print('ONNX export success, saved as detr.onnx')
